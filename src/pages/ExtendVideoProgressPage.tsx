...
import { CheckCircle2, Clock, Loader2, Maximize2, Film, Scissors, Layers, Sparkles } from "lucide-react";
import confetti from "canvas-confetti";

const STEPS = [
  { target: 20, duration: 15000, label: "Analyzing original video...", icon: Film },
  { target: 45, duration: 25000, label: "Generating continuation script...", icon: Sparkles },
  { target: 65, duration: 30000, label: "Creating extended scene...", icon: Scissors },
  { target: 85, duration: 40000, label: "Merging video segments...", icon: Layers },
  { target: 90, duration: 60000, label: "Almost ready, final rendering...", icon: Clock },
] as const;
...
export default function ExtendVideoProgressPage() {
  ...
  // Check if extension already completed (new record with parent_video_id = id exists with status done)
  useEffect(() => {
    if (!id) return;
    supabase
      .from("videos")
  ...
  // Realtime subscription — listen for new INSERT with parent_video_id = id
  useEffect(() => {
    if (!id) return;
    const channel = supabase
      .channel(`extend-progress-${id}`)
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "videos", filter: `parent_video_id=eq.${id}` },
        (payload) => {
        ...
        <div className="glass-card p-8 md:p-10 space-y-8 relative overflow-hidden">
          ...
          <div className="flex justify-center relative z-10">
            <AnimatePresence mode="wait">
              {done ? (
                ...
                >
                  <Maximize2 className="h-20 w-20 text-primary" />
                  <motion.div
                    className="absolute inset-0 rounded-full"
                    style={{ boxShadow: "0 0 40px hsl(var(--primary) / 0.4), 0 0 80px hsl(260 60% 50% / 0.25)" }}
          ...
          <div className="text-center relative z-10 space-y-2">
            <h1 className="text-2xl md:text-3xl font-bold text-foreground font-display">
              {done ? "Your extended video is ready! 🎉" : "Extending Your Video ✨"}
            </h1>
            {productName && (
              <p
                className="text-lg font-semibold"
                style={{
                  background: "linear-gradient(90deg, hsl(var(--primary)), hsl(260 60% 60%))",
            ...
            <div className="relative h-4 w-full rounded-full bg-muted/50 overflow-hidden border border-border/50">
              <motion.div
                ...
                style={{
                  background: done
                    ? "hsl(var(--success))"
                    : "linear-gradient(90deg, hsl(var(--primary)), hsl(260 60% 55%))",
                  boxShadow: done
          ...
          >
            ...
            <p className="text-lg font-bold text-foreground">~4 minutes</p>
            <p className="text-xs text-muted-foreground/60">to extend your video</p>
          </motion.div>
        </div>
      </motion.div>
    </div>
